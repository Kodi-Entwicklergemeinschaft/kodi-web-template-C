...
const TodaysEvents = () => {
  const { listingDetails: todaysEventsData , isLoading} = useGetListingsWithCategories(
    {
    ...
    <Box sx={{ px: 6, py: 8 }}>
      <Grid container spacing={6} alignItems="flex-start" direction={{ xs: 'column-reverse', sm: 'column-reverse', md: 'row' }} >
              ...
              <Stack spacing={3}>
                ...
                {todaysEventsData.length === 0 && (
                  <Typography variant="body1" sx={{ mb: 4 }}>
                    {t('todaysEvents.fallback_message')}
                  </Typography>
                )}
              </Stack>
            </Box>
        ...
        <Grid item xs={12} md={5}>
          <Typography variant="h4" sx={{ mb: 4 , ml:8 }} fontWeight={600}>
            {t('todaysEvents.title')}
          </Typography>
          <Typography variant="body2" sx={{ mb: 2 , ml:8 }}>
            {t('todaysEvents.description1')}
          </Typography>
          <Typography variant="body2" sx={{ mb: 3 , ml:8 }}>
            {t('todaysEvents.description2')}
          </Typography>
          <Typography variant="body2" sx={{ mb: 4 , ml:8}}>
            {t('todaysEvents.description3')}
          </Typography>
          <Stack direction="row" spacing={2} gap={2} sx={{ ml:8 }}>
            <Button
              variant="contained"
